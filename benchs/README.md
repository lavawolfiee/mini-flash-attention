...
The benchmark measures forward-only attention runtime with CUDA events and peak allocated CUDA memory. All calls run under `torch.inference_mode()` with `causal=False` and `dropout_p=0.0` for FlashAttention backends.

FA1 and FA2 run in separate virtual environments because both install a `flash_attn` Python package with different compiled extensions. Runtime plots and TFLOPs/s use median latency; mean latency is kept in JSONL for reference.
...
.venv/bin/python benchs/benchmark_forward.py --backend wmma --out benchs/results/wmma.jsonl
